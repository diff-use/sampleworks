    ...
    with ProcessPoolExecutor(
        ...
        for worker_num, job_queue_path in enumerate(job_queue_paths):
            future = executor.submit(
                ...
                (job_queue_path, max_workers, worker_job_queues, worker_num)
            )
...
def run_guidance_queue_script(args: tuple[str, int, list[list[JobConfig]], int]):
    job_queue_path, max_workers, worker_job_queues, worker_num = args
    pixi_env = get_pixi_env(worker_job_queues[worker_num][0].model)
    ...
    env = os.environ.copy()
    env["CUDA_VISIBLE_DEVICES"] = f"cuda:{worker_num % max_workers}"
