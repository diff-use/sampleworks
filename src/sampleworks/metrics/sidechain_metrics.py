"""
This file originated in RosettaCommons/foundry/models/rfd3 and is licensed under BSD-3-Clause.
"""
...
def collapsing_virtual_atoms_batched(
    ...
    for atom_array in atom_arrays:
        ...
        chain_iid_and_res_id = np.char.add(chain_iid_with_sep, atom_array.res_id.astype(str))
        ...
        for res_identifier in unique_residue_identifiers:
            ...
            # The current way to handle this is to check if predicted CA and CB are too close,
            # because in the case of glycine and we pad virtual atoms based on CB, CB's coords
            # are set as CA.
...
# TODO: this is imported directly from RosettaCommons/foundry/models/rfd3; it needs to be
#  refactored into smaller functional pieces.
def check_sidechain_quality(
    atom_array, dist_threshold_min: float = 1.0, dist_threshold_max: float = 2.0
):
    """
    ...
    Return:
     ...
     - unintended_bonds_percent (float): percentage of sidechains with
           unintended bonds with other sidechains.
    ...
    standard_aa_atom_array = [
        aa[(~np.isin(aa.atom_name, np.array(["OXT"]))) & (aa.element != "H")]  # pyright: ignore
    ...
    for res_identifier in unique_residue_identifiers:
        ...
        cur_res_coords = atom_array.coord[atom_array.chain_iid_and_res_id == res_identifier]
        ...
        try:
            ...
            print(f"[WARN] Could not build graph for chain_iid|res_id {res_identifier}: {e}")
            continue
    ...
    unintended_bonds = {res_identifier: False for res_identifier in unique_residue_identifiers}
    N = dists.shape[0]
    ...
    potential_bonds = (dists[iu, ju] > dist_threshold_min) & (dists[iu, ju] < dist_threshold_max)
    ...
    clash_residues = {res_identifier: False for res_identifier in unique_residue_identifiers}
    ...
    if_unintended_bonds = [
        unintended_bonds[res_identifier] for res_identifier in unique_residue_identifiers
    ]
    if_clash = [clash_residues[res_identifier] for res_identifier in unique_residue_identifiers]

    valid_sidechain_percent = sum(if_valid_sidechains) / len(unique_residue_identifiers)
    unintended_bonds_percent = sum(if_unintended_bonds) / len(unique_residue_identifiers)
...
class SidechainMetrics(Metric):
    ...
    def compute(self, predicted_atom_array_stack):
        ...
        o = {
            ...
            "mean_clash_percent": float(np.mean([m["clash_percent"] for m in batch_metrics])),
