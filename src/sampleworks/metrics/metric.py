...
We have removed support for Foundry's RankedLogger.
"""
...
def instantiate_metric_manager(
    ...
    for name, cfg in metrics_cfg.items():
        ...
        logger.info(f"Adding metric {name} to the validation metrics...")
        metrics[name] = metric
...
class MetricManager:
    ...
    def from_metrics(
        ...
        for name, metric in metrics.items():
            if not isinstance(metric, Metric):
                raise TypeError(f"Metric '{name}' must be a Metric instance, got {type(metric)}")
    ...
    def __call__(self, **kwargs: Any) -> dict[str, Any]:
        """
        Compute all metrics and return their results as a
        dictionary mapping metric names to their computed values.
        """

        # Extract example_id if it exists.
        # we have to "fix" kwargs here because this method expects a tuple of str for keys.
        #  even though the actual method handles it just fine.
        example_id = nested_dict.get(
            {(k,): v for k, v in kwargs.items()}, key=("extra_info", "example_id"), default=None
        )
        ...
        for name, metric in self.metrics.items():
            ...
            example_msg = f" for example '{example_id}'" if example_id is not None else ""
            ...
            if "extra_info" in kwargs and "metrics_tags" in kwargs["extra_info"]:
                ...
                if metric.required_tags_any and not metrics_tags.intersection(
                    ...
                    continue
                if metric.prohibited_tags and metrics_tags.intersection(metric.prohibited_tags):
                    continue
...
class Metric(ABC):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.required_tags_all = set(required_tags_all) if required_tags_all is not None else set()
        self.required_tags_any = set(required_tags_any) if required_tags_any is not None else set()
        self.prohibited_tags = set(prohibited_tags) if prohibited_tags is not None else set()
        ...
        # Check that the 'keys' of the kwargs_to_compute_args are a
        # subset of the 'compute' method signature
        if self.kwargs_to_compute_args:
            allowable_kwargs = self.kwargs_to_compute_args.keys()
            assert allowable_kwargs <= self.required_compute_args, (
                f"The keys of kwargs_to_compute_args must be a subset of the 'compute' "
                f"method signature. {allowable_kwargs} is not a "
                f"subset of {self.required_compute_args}"
            )

        # Check that optional_kwargs are also in the kwargs_to_compute_args
        if self.kwargs_to_compute_args and self.optional_kwargs:
            allowable_kwargs = self.kwargs_to_compute_args.keys()
            assert self.optional_kwargs <= set(allowable_kwargs), (
                f"All optional_kwargs must be defined in kwargs_to_compute_args. "
                f"{self.optional_kwargs} is not a subset of {set(allowable_kwargs)}"
            )
                ...
                if compute_arg in self.optional_kwargs:
                    ...
                    try:
                        compute_inputs[compute_arg] = nested_dict.getitem(
                            {(k,): v for k, v in kwargs.items()}, key=kwargs_key
                        )
                    except KeyError:
                        pass  # Don't pass this parameter to compute()
                else:
                    # Required parameter - use getitem (will raise if missing)
                    # again, have to coax the kwargs into the format of nested_dict.get's signature
                    compute_inputs[compute_arg] = nested_dict.getitem(
                        {(k,): v for k, v in kwargs.items()}, key=kwargs_key
                    )
        else:
    ...
    @abstractmethod
    def compute(self, *args: Any, **kwargs: Any) -> dict[str, Any] | list[dict[str, Any]]:
