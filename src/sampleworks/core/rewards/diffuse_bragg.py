...
class DiffuseBraggRewardFunction:
        ...
        *,
        ...
        if callable(bragg_weight):
            ...
            if not 0.0 <= float(bragg_weight) <= 1.0:
                raise ValueError(
                    f"bragg_weight must lie in [0, 1] to be a convex mixture; "
                    f"got {bragg_weight}."
                )
    ...
    def _finite_column(dataset: rs.DataSet, column: str) -> tuple[np.ndarray, np.ndarray]:
        ...
        hkl = np.column_stack([dataset.index.get_level_values(i).to_numpy() for i in ("H", "K", "L")])
    ...
    def __call__(
        ...
        f_configs = lunus_sf.structure_factors(
            self.setup, coordinates, occupancies, self._hkl_t
        )
