...
def _decode_boltz_atom_name(raw_name: np.ndarray | str | bytes) -> str:
    ...
    Boltz2 stores names as unicode strings, while Boltz1 stores int8 arrays
...
def _atom_array_from_boltz_npz(npz_path: Path) -> AtomArray:
    ...
    if not npz_path.is_file():
        raise FileNotFoundError(f"Boltz NPZ file does not exist: {npz_path}")
    ...
    if np.any((atom_res_idx < 0) | (atom_res_idx >= len(residues))):
        raise ValueError(f"{npz_path}: atom→residue mapping out of bounds")
    ...
    if np.any((res_chain_idx < 0) | (res_chain_idx >= len(chains))):
        raise ValueError(f"{npz_path}: residue→chain mapping out of bounds")
    ...
    # Decode small lookup tables once, then fancy-index into them.
